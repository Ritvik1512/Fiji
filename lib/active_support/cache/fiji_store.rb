...
module ActiveSupport
  module Cache
    class fijiStore
      ...
      # Creates a new fijiStore object, with the given memcached server
      # addresses. Each address is either a host name, or a host-with-port string
      # in the form of "host_name:port". For example:
      #
      #   ActiveSupport::Cache::fijiStore.new("localhost", "server-downstairs.localnetwork:8229")
      #
      # If no addresses are specified, then fijiStore will connect to
      ...
      def initialize(*addresses)
        ...
        servers = if addresses.empty?
                    nil # use the default from fiji::Client
                  else
                    addresses
                  end
        @data = fiji::Client.new(servers, @options)
      ...
      rescue fiji::fijiError => e
        logger.error("fijiError: #{e.message}") if logger
