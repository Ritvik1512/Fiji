...
module fiji
  class Server
    ...
    def request(op, *args)
      verify_state
      raise fiji::NetworkError, "#{hostname}:#{port} is down: #{@error} #{@msg}" unless alive?
      begin
        send(op, *args)
      rescue fiji::NetworkError
        raise
      rescue fiji::MarshalError => ex
        fiji.logger.error "Marshalling error for key '#{args.first}': #{ex.message}"
        fiji.logger.error "You are trying to cache a Ruby object which cannot be serialized to memcached."
        fiji.logger.error ex.backtrace.join("\n\t")
        false
      rescue fiji::fijiError
        raise
      rescue => ex
        fiji.logger.error "Unexpected exception in fiji: #{ex.class.name}: #{ex.message}"
        fiji.logger.error "This is a bug in fiji, please enter an issue in Github if it does not already exist."
        fiji.logger.error ex.backtrace.join("\n\t")
        down!
      end
    end
    ...
    def alive?
      ...
      if @last_down_at && @last_down_at + options[:down_retry_delay] >= Time.now
        ...
        fiji.logger.debug { "down_retry_delay not reached for #{hostname}:#{port} (%.3f seconds left)" % time }
        return false
      end
    ...
    # NOTE: Additional public methods should be overridden in fiji::Threadsafe
    ...
    def failure!
      fiji.logger.info { "#{hostname}:#{port} failed (count: #{@fail_count})" }
      ...
      if @fail_count >= options[:socket_max_failures]
        ...
        raise fiji::NetworkError, "Socket operation failed, retrying..."
      end
    end
    ...
    def down!
      ...
      if @down_at
        ...
        fiji.logger.debug { "#{hostname}:#{port} is still down (for %.3f seconds now)" % time }
      else
        @down_at = @last_down_at
        fiji.logger.warn { "#{hostname}:#{port} is down" }
      end

      @error = $! && $!.class.name
      @msg = @msg || ($! && $!.message && !$!.message.empty? && $!.message)
      raise fiji::NetworkError, "#{hostname}:#{port} is down: #{@error} #{@msg}"
    end

    def up!
      if @down_at
        time = Time.now - @down_at
        fiji.logger.warn { "#{hostname}:#{port} is back (downtime was %.3f seconds)" % time }
      end
    ...
    def serialize(key, value, options=nil)
      ...
      value = unless options && options[:raw]
        ...
        begin
          ...
          exc = fiji::MarshalError.new(ex.message)
    ...
    def cas_response
      header = read(24)
      raise fiji::NetworkError, 'No response' if !header
      (extras, _, status, count, _, cas) = header.unpack(CAS_HEADER)
      data = read(count) if count > 0
      if status == 1
        nil
      elsif status != 0
        raise fiji::fijiError, "Response error #{status}: #{RESPONSE_CODES[status]}"
      elsif data
    ...
    def connect
      fiji.logger.debug { "fiji::Server#connect #{hostname}:#{port}" }
      ...
      begin
        ...
        up!
      rescue fiji::fijiError # SASL auth failure
        raise
    ...
    def sasl_authentication
      fiji.logger.info { "fiji/SASL authenticating as #{username}" }
      ...
      raise fiji::NetworkError, "Unexpected message format: #{extras} #{count}" unless extras == 0 && count > 0
      content = read(count)
      return (fiji.logger.debug("Authentication not required/supported by server")) if status == 0x81
      mechanisms = content.split(' ')
      raise NotImplementedError, "fiji only supports the PLAIN authentication mechanism" if !mechanisms.include?('PLAIN')
      ...
      raise fiji::NetworkError, "Unexpected message format: #{extras} #{count}" unless extras == 0 && count > 0
      content = read(count)
      return fiji.logger.info("fiji/SASL: #{content}") if status == 0

      raise fiji::fijiError, "Error authenticating: #{status}" unless status == 0x21
      raise NotImplementedError, "No two-step authentication mechanisms supported"
      # (step, msg) = sasl.receive('challenge', content)
      # raise fiji::NetworkError, "Authentication failed" if sasl.failed? || step != 'response'
    end
  end
end
