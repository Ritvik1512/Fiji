...
module fiji
  class Client
    ...
    ##
    # fiji::Client is the main class which developers will use to interact with
    # the memcached server.  Usage:
    #
    #   fiji::Client.new(['localhost:11211:10', 'cache-2.example.com:11211:5', '192.168.0.1:22122:5'],
    #                   :threadsafe => true, :failover => true, :expires_in => 300)
    #
    # servers is an Array of "host:port:weight" where weight allows you to distribute cache unevenly.
    # Both weight and port are optional.  If you pass in nil, fiji will use the <tt>MEMCACHE_SERVERS</tt>
    ...
    # - :compress - defaults to false, if true fiji will compress values larger than 1024 bytes before
    ...
    # pipelined as fiji will use 'quiet' operations where possible.
    # Currently supports the set, add, replace and delete operations.
    def multi
      old, Thread.current[:fiji_multi] = Thread.current[:fiji_multi], true
      yield
    ensure
      Thread.current[:fiji_multi] = old
    end
    ...
    def get_multi(*keys)
      ...
      ring.lock do
        ...
        keys.flatten.each do |key|
          begin
            perform(:getkq, key)
          rescue fijiError, NetworkError => e
            fiji.logger.debug { e.inspect }
            fiji.logger.debug { "unable to get key #{key}" }
          end
        end
        ...
        servers_in_use.each do |server|
          ...
          begin
            ...
            fiji.logger.debug { e.inspect }
            fiji.logger.debug { "results from this server will be missing" }
          end
        end
        values
      end
      ...
      @ring ||= fiji::Ring.new(
        Array(@servers).map do |s|
         server_options = {}
          ...
          fiji::Server.new(s, @options.merge(server_options))
        end, @options
      )
    end
    ...
    def perform(op, key, *args)
      ...
      begin
        ...
        fiji.logger.debug { e.inspect }
        fiji.logger.debug { "retrying request with new server" }
        retry
      end
    end
    ...
    def normalize_options(opts)
      if opts[:compression]
        fiji.logger.warn "DEPRECATED: fiji's :compression option is now just :compress => true.  Please update your configuration."
